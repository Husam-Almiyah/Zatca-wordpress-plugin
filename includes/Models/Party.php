...
class Party implements XmlSerializable
{
    ...
    public function setPartyIdentificationId(?string $partyIdentificationId): self
    {
        ...
        // Enforce one of the allowed scheme IDs: CRN, MOM, MLS, SAG, OTH, 700
        $allowed = ['CRN','MOM','MLS','SAG','OTH','700'];
        if ($partyIdentificationId !== null && !in_array($partyIdentificationId, $allowed, true)) {
            // Default to CRN if invalid scheme provided
            $this->partyIdentificationId = 'CRN';
        } else {
